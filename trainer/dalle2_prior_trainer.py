...
from models.dalle2_prior import DiffusionPrior
from accelerate import Accelerator, DistributedType
from models.dalle2_utils import exists, default
from functools import partial, wraps
from dalle2_pytorch.train import cast_tuple, get_optimizer, GradScaler, autocast
...
class DiffusionPriorTrainer(nn.Module):
    def __init__(
        ...
        self.optim_kwargs = dict(lr=lr, wd=wd)
        ...
        self.warmup_scheduler = warmup.LinearWarmup(self.optimizer, warmup_period=warmup_steps) if exists(
            warmup_steps) else None

        # distribute the model if using HFA

        self.diffusion_prior, self.optimizer, self.scheduler = self.accelerator.prepare(self.diffusion_prior,
            ...
            if self.use_ema:
                save_obj = {
                    **save_obj,
                    ...
                    'ema_model': self.ema_diffusion_prior.ema_model.state_dict()
                    # save the ema model specifically for easy ema-only reload
                }
